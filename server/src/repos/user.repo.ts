...
export default class UserRepository {
  ...
  static async create({
    ...
    try {
      ...
      // Try to insert with status field, if it fails, try without it (for backward compatibility)
      try {
        ...
        console.log("Status column may not exist, trying without it:", statusError.message);
        const result = await query(
          `INSERT INTO users
