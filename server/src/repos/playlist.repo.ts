    ...
    try {
      ...
      const processedSongs = await Promise.all(
        songs.map(async (song: PlaylistSong) => {
          ...
          if (song.albums?.length && song.albums[0]?.image_url) {
            // Type assertions used to satisfy strict null checks
            song.albums[0].image_url = getBlobUrl(song.albums[0].image_url as string);
          }
          return song;
        })
      );
