...
const DataRequestForm: React.FC<Props> = ({ onSubmit }) => {
  ...
  const initializeDefaultParameters = (
    type: ReportType,
    reportTypesData: Record<string, any>
  ) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!reportType || !dateFrom || !dateTo) {
      alert(
        "Please fill in all required fields: Report Type, From Date, and To Date"
      );
      return;
    }
    ...
    if (
      ...
      alert(
        "Results Limit cannot exceed 50. Please enter a value between 1 and 50."
      );
      return;
    }
