...
import { AdminAppealsAPI, type AdminAppeal, type AppealEntityType } from "../../api/admin-appeals.api"; 
...
const AdminAppealsPage: React.FC = () => {
  ...
  async function handleAction(appeal: AdminAppeal, action: "approve" | "reject") {
    try {
      setLoading(true);
      ...
      const response = await AdminAppealsAPI.decideAppeal(appealType, appealDecisionData);

      if (response.success) {
        // Remove the processed appeal from the local state
        setAppeals(prevAppeals => 
          prevAppeals.filter(a => 
      ...
      <main className={styles.contentArea}>
        <div className={styles.contentWrapper}>
          ...
          <div className={styles.appealsDropdownContainer}>
            <AppealsDropdown selectedAppeal={appealType} onSelect={setAppealType} />
          </div>
