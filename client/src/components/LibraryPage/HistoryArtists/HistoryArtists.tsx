...
import styles from "./HistoryArtists.module.css";

const HistoryArtists: React.FC<{
  userId: UUID;
  searchFilter?: string;
}> = ({ userId, searchFilter = "" }) => {
  const { data, loading, error } = useAsyncData(
    {
      artists: () =>
        libraryApi.getArtistHistory(userId, { timeRange: "1 month" }),
    },
    [userId],
    {
      cacheKey: `history_artists_${userId}`,
      hasBlobUrl: true,
    }
  );
    ...
    <>
      {filteredArtists.length === 0 ? (
        ...
        <div className={styles.sectionContainer}>
          <span className={styles.sectionTitle}>Artist History</span>
...
export default memo(HistoryArtists);
